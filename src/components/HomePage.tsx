...
interface ActivationInfo { id: number; title: { [key: string]: string }; description: { [key: string]: string }; category: { [key: string]: string }; image_url?: string; price?: string; ecu?: string; }
interface CartItem extends ActivationInfo { carModelName: string; carYear: string; }

const getOpeningHoursStatus = (lang: 'ar' | 'en' | 'he') => {
    const now = new Date(); const day = now.getDay(); const hour = now.getHours(); const t = translations[lang]; let isOpen = false;
...
  useEffect(() => { document.documentElement.dir = language === 'en' ? 'ltr' : 'rtl'; document.documentElement.lang = language; }, [language]);
  useEffect(() => { const interval = setInterval(() => setCurrentBgIndex(p => (p + 1) % backgroundImages.length), 5000); return () => clearInterval(interval); }, []);
  useEffect(() => {
...
    setContactMessage(`${carInfo}${cartItems}`);
...
  };
  
  const showDetail = (act: ActivationInfo) => { setCurrentDetailedActivation(act); setLlmResponse(null); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  const removeFromCart = (id: number) => setCart(cart.filter(item => item.id !== id));
  const calculateTotal = () => cart.reduce((total, item) => total + (item.price ? parseInt(item.price) : 0), 0);
...
      if(response.ok) { setContactName(''); setContactEmail(''); setContactPhone(''); }
    } catch (error) { alert(t.contactNetworkError); }
  };
...
    setLlmLoading(true); setLlmResponse(null); await new Promise(resolve => setTimeout(resolve, 1500));
    let simulatedContent = currentDetailedActivation.description?.[language] || t.detailNoSteps;
    setLlmResponse({ title: t[titleKey], content: simulatedContent }); setLlmLoading(false);
  };
...
  }, [selectedCar]);
  
  if (currentDetailedActivation) {
      return (
          <div className="min-h-screen bg-gradient-to-br from-blue-900 to-black text-white font-sans p-4 sm:p-8" dir={language === 'en' ? 'ltr' : 'rtl'}>
              <div className="max-w-4xl mx-auto p-6 sm:p-8 bg-white/10 backdrop-blur-lg rounded-lg shadow-2xl animate-fadeIn">
...
                    {llmResponse && (<div className="bg-gray-800 rounded-lg p-4 mt-6 border border-gray-700"><h4 className="text-xl font-semibold mb-2 text-blue-400">{llmResponse.title}</h4><p className="text-gray-300 whitespace-pre-wrap">{llmResponse.content}</p></div>)}
              </div>
          </div>
      )
  }
...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-900 to-black text-white font-sans">
      <div className="fixed inset-0 -z-20 overflow-hidden"><img src={backgroundImages[currentBgIndex]} alt="Background" className="w-full h-full object-cover transition-opacity duration-1000" style={{ opacity: 0.25 }} /><div className="absolute inset-0 bg-gradient-to-br from-blue-900/80 to-black/80"></div></div>
      <header className="bg-transparent backdrop-blur-md p-4 shadow-lg sticky top-0 z-50"><div className="container mx-auto flex justify-between items-center"><div className="flex items-center"><div className="w-12 h-12 bg-contain bg-center bg-no-repeat" style={{ backgroundImage: 'url(https://upload.wikimedia.org/wikipedia/commons/4/44/BMW.svg)' }}></div><h1 className="text-2xl font-bold mx-3"><span className="text-blue-500">{t.brandName1}</span>///<span className="text-red-500">{t.brandName2}</span></h1></div><nav className="hidden md:flex items-center"><ul className="flex items-center gap-x-6"><li><button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="hover:text-blue-400 transition">{t.headerHome}</button></li><li><button onClick={() => activationsRef.current?.scrollIntoView({ behavior: 'smooth' })} className="hover:text-blue-400 transition">{t.headerActivations}</button></li><li><button onClick={() => servicesRef.current?.scrollIntoView({ behavior: 'smooth' })} className="hover:text-blue-400 transition">{t.headerServices}</button></li><li><button onClick={() => contactRef.current?.scrollIntoView({ behavior: 'smooth' })} className="hover:text-blue-400 transition">{t.headerContact}</button></li></ul></nav><div className="flex items-center gap-x-4"><div className="flex items-center gap-x-2 text-sm"><button onClick={() => setLanguage('en')} className={`px-2 py-1 rounded transition-colors ${language==='en'?'bg-blue-600':'bg-white/10 hover:bg-white/20'}`}>EN</button><button onClick={() => setLanguage('he')} className={`px-2 py-1 rounded transition-colors ${language==='he'?'bg-blue-600':'bg-white/10 hover:bg-white/20'}`}>עב</button><button onClick={() => setLanguage('ar')} className={`px-2 py-1 rounded transition-colors ${language==='ar'?'bg-blue-600':'bg-white/10 hover:bg-white/20'}`}>AR</button></div><button onClick={() => setIsCartOpen(true)} className="relative hover:text-blue-400 transition"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" /></svg>{cart.length > 0 && <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">{cart.length}</span>}</button></div></div></header>
...
          <section className="text-center mb-16"><h1 className="text-4xl md:text-6xl font-bold mb-4"><span className="text-blue-500">{t.brandName1}</span>///<span className="text-red-500">{t.brandName2}</span></h1><p className="text-xl text-gray-300 max-w-3xl mx-auto">{t.welcomeTitle}</p></section>
          <section className="mb-16"><ModelSelector onSelectionChange={handleSelectionChange} /></section>
          <section ref={activationsRef} className="mb-16"><div className="bg-white/5 backdrop-blur-md rounded-lg p-6 shadow-xl"><h2 className="text-3xl font-bold mb-8 text-center">{t.activationsTitle}{selectedCar && <span className="block text-xl text-blue-300 mt-2">{t.activationsFor} {selectedCar.modelName}</span>}</h2><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
...
              })}
          </div></div></section>
          <section ref={servicesRef} className="mb-16"><h2 className="text-3xl font-bold mb-8 text-center">{t.servicesTitle}</h2><div className="grid grid-cols-1 md:grid-cols-3 gap-8"><div className="bg-white/5 backdrop-blur-md rounded-lg p-6 border border-white/10 text-center"><h3 className="text-xl font-bold mb-2">{t.service1Title}</h3><p className="text-gray-300">{t.service1Desc}</p></div><div className="bg-white/5 backdrop-blur-md rounded-lg p-6 border border-white/10 text-center"><h3 className="text-xl font-bold mb-2">{t.service2Title}</h3><p className="text-gray-300">{t.service2Desc}</p></div><div className="bg-white/5 backdrop-blur-md rounded-lg p-6 border border-white/10 text-center"><h3 className="text-xl font-bold mb-2">{t.service3Title}</h3><p className="text-gray-300">{t.service3Desc}</p></div></div></section>
          <section id="contact" ref={contactRef} className="bg-white/5 backdrop-blur-md rounded-lg p-8 shadow-xl"><h2 className="text-3xl font-bold mb-8 text-center">{t.contactTitle}</h2><div className="grid grid-cols-1 md:grid-cols-2 gap-12"><div className="space-y-6"><div><h3 className="text-xl font-semibold mb-4">{t.contactInfo}</h3><div className="space-y-3"><p><strong>{t.contactPhone}:</strong><a href="tel:+972528180757" className="hover:text-blue-400 mx-2">+972 52 818 0757</a></p><p><strong>{t.contactEmail}:</strong><a href="mailto:a.3cx.92@Gmail.com" className="hover:text-blue-400 mx-2">a.3cx.92@Gmail.com</a></p><p><strong>{t.contactAddress}:</strong> الطيرة المثلث, إسرائيل</p></div></div><div><h3 className="text-xl font-semibold mb-4">{t.businessHours}</h3><div className="space-y-2"><p className="text-gray-300">{t.hoursSundayThursday}</p><p className="text-gray-300">{t.hoursFriday}</p><p className="text-gray-300">{t.hoursSaturday}</p><div className={`mt-4 text-lg font-bold p-3 rounded-md text-center ${openingStatus.isOpen ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>{openingStatus.message}</div></div></div></div><div><h3 className="text-xl font-semibold mb-4">{t.contactSendMessage}</h3><form className="space-y-4" onSubmit={handleSubmit}><input type="text" placeholder={t.contactFullName} className="w-full bg-white/10 border border-white/20 rounded-md px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500" value={contactName} onChange={e=>setContactName(e.target.value)} required/><input type="email" placeholder={t.contactYourEmail} className="w-full bg-white/10 border border-white/20 rounded-md px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500" value={contactEmail} onChange={e=>setContactEmail(e.target.value)} required/><input type="tel" placeholder={t.contactPhoneNumber} className="w-full bg-white/10 border border-white/20 rounded-md px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500" value={contactPhone} onChange={e=>setContactPhone(e.target.value)}/><textarea rows={6} placeholder={t.contactMessage} className="w-full bg-white/10 border border-white/20 rounded-md px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500" value={contactMessage} onChange={e=>setContactMessage(e.target.value)} required></textarea><button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 py-3 rounded-md transition-transform transform hover:scale-105">{t.contactSubmit}</button></form></div></div></section>
      </main>
      <footer className="bg-black/50 backdrop-blur-md py-8 mt-16"><div className="container mx-auto px-4 text-center"><h2 className="text-2xl font-bold"><span className="text-blue-500">{t.brandName1}</span>///<span className="text-red-500">{t.brandName2}</span></h2><p className="text-gray-400 mt-2">{t.footerSlogan}</p><div className="mt-8 border-t border-gray-800 pt-6 text-gray-500 text-sm"><p>{t.footerRights(new Date().getFullYear())}</p></div></div></footer>
    </div>
  );
};
